...
class GraphSearch(SearchProblem):
    ...
    def __init__(self, graph_text):
        ...
        if r == None:
            print("Broken graph:")
            print('"""%s"""' % graph_text)
            raise Exception("GraphSearch graph specification start_state not found or incorrect on line:" )
        self.start_state = r.group(1).strip()
        r = re.match('goal_states:(.*)', lines[1])
        if r == None:
            print("Broken graph:")
            print('"""%s"""' % graph_text)
            raise Exception("GraphSearch graph specification goal_states not found or incorrect on line:")
...
#     f.close()
def timeLimitedHeuristic(heur, state, problem):
    start = time.time()
    result =  heur(state, problem )
    end = time.time()
    #print(end - start)
    if end - start > 0.001:
        raise TimeoutError
...
class HeuristicTest(testClasses.TestCase):
    ...
    def setupProblem(self, searchAgents):
        ...
        tHeuristic = lambda x,y: timeLimitedHeuristic(heuristic, x,y)

        return problem, state, tHeuristic
